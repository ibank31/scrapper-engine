...
def main() -> None:
    ...
    ap.add_argument("--model", default=os.environ.get("CLIPPER_WHISPER_MODEL", "small"), help="tiny/base/small/medium/large-v3/turbo")
    ap.add_argument("--language", default=None)
    ap.add_argument("--device", default="cpu", choices=("cpu", "cuda"))
    ap.add_argument("--compute-type", default=None, help="default: int8 on CPU, float16 on CUDA")
    ap.add_argument("--beam-size", type=int, default=int(os.environ.get("CLIPPER_WHISPER_BEAM", "5")), help="1=fastest, 5=default quality")
    args = ap.parse_args()
    try:
    ...
    beam_size = max(1, min(5, int(args.beam_size)))
    ...
    payload = {"input": os.path.abspath(args.input), "model": args.model, "device": args.device, "compute_type": compute_type, "beam_size": beam_size, "language": info.language, "language_probability": info.language_probability, "segments": result_segments}
    ...
    with open(os.path.join(out_dir, "transcript.srt"), "w", encoding="utf-8") as fh:
        fh.write("\n".join(srt_lines))
    print("OK:", out_dir, "| segments:", len(result_segments), "| language:", info.language, "| model:", args.model, "| beam:", beam_size)
